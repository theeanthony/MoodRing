...
public class ChromaColorPicker: UIControl, ChromaControlStylable {
    ...
    public func deleteHandle(_ handle: ChromaColorHandle) {
        // Check if the handle is part of the handles array
        if let index = handles.firstIndex(where: { $0 === handle }) {
            // Remove the handle from the array
            handles.remove(at: index)
            
            // Remove the handle from the view hierarchy
            handle.removeFromSuperview()
            
            // Optionally, update the current handle if it was the one removed
            if handle === currentHandle {
                currentHandle = handles.first // or nil, depending on your logic
            }
        }
    }
